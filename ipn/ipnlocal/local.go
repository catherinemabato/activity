...
// A zero value or a nil pointer are valid and indicate that
// the notification should be broadcast to all active [watchSession]s.
...
// newNotificationTarget returns a new [notificationTarget] that matches only actors
...
func newNotificationTarget(actor ipnauth.Actor) *notificationTarget {
...
func (b *LocalBackend) popBrowserAuthNow(url string, keyExpired bool, recipient ipnauth.Actor) {
	...
	b.tellRecipientToBrowseToURL(url, newNotificationTarget(recipient))
