...
func TestGetResultantPolicyNoSource(t *testing.T) {
	tests := []struct {
		name  string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("Failed to get resultant policy for %v: %v", tt.scope, err)
			}
...
func TestPolicyFor(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			policyA, err := resultantPolicyForTest(t, tt.scopeA)
			if err != nil {
				t.Fatalf("Failed to get resultant policy for %v: %v", tt.scopeA, err)
			}
			...
			policyB, err := resultantPolicyForTest(t, tt.scopeB)
			if err != nil {
				t.Fatalf("Failed to get resultant policy for %v: %v", tt.scopeB, err)
			}
...
func TestChangePolicySetting(t *testing.T) {
	...
	// Register a test policy store and create a resultant policy that reads the policy settings from it.
	...
	policy, err := resultantPolicyForTest(t, setting.DeviceScope)
	if err != nil {
		t.Fatalf("Failed to get resultant policy: %v", err)
	}
...
drain:
	...
	// Lastly, if a policy store issues a change notification, but the resultant policy
...
func TestClosePolicySource(t *testing.T) {
	...
	if _, err := policy.Reload(); err == nil || !errors.Is(err, ErrResultantPolicyClosed) {
		t.Fatalf("Reload: gotErr: %v; wantErr: %v", err, ErrResultantPolicyClosed)
	}
}
...
func TestRemovePolicySource(t *testing.T) {
	...
	// Create a resultant [Policy] that reads policy settings from the two stores.
	policy, err := resultantPolicyForTest(t, setting.DeviceScope)
	if err != nil {
		t.Fatalf("Failed to get resultant policy: %v", err)
	}
	...
	// Unregister Store A and verify that the resultant policy remains valid.
	...
	// Unregister Store B and verify that the resultant policy is still valid.
...
func TestReplacePolicySource(t *testing.T) {
	...
	// Register the initial store and create a resultant [Policy] that reads policy settings from it.
...
func TestAddClosedPolicySource(t *testing.T) {
	...
	_, err := resultantPolicyForTest(t, setting.DeviceScope)
...
func checkPolicySources(tb testing.TB, gotPolicy *Policy, wantSources []*source.Source) {
	...
	// cmp.FilterPath(func(p cmp.Path) bool { return p.String() == ""})
	// var i int
	// for i = range min(len(gotPolicy.sources), len(wantSources)) {
	// 	gotSource := gotPolicy.sources[i].Source
	// 	if gotSource != wantSources[i] {
	// 		tb.Errorf("Sources[%d]: got %q (%v); want %q (%v)", i,
	// 			gotSource.Name(), gotSource.Scope(),
	// 			wantSources[i].Name(), wantSources[i].Scope(),
	// 		)
	// 	}
	// }
}
