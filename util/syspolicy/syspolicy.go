...
// RegisterChangeCallback adds a function that will be called whenever the resultant policy
...
func RegisterChangeCallback(cb rsop.PolicyChangeCallback) (unregister func(), err error) {
	resultant, err := rsop.PolicyFor(setting.DefaultScope())
	if err != nil {
		return nil, err
	}
	return resultant.RegisterChangeCallback(cb), nil
}
